	...
	def loss(self, X, y=None):
		...
		if self.num_layers == 1:
			...
			for l in range(self.num_layers-2):
				out, self.params['cache_a'+str(l+1)] = affine_forward(out, self.params['W'+str(l+1)], self.params['b'+str(l+1)])
				out, self.params['cache_r'+str(l+1)] = relu_forward(out)

			preds, self.params['cache_a'+str(self.num_layers-1)] = affine_forward(out, self.params['W'+str(self.num_layers-1)], self.params['b'+str(self.num_layers-1)])
		############################################################################
		#                             END OF YOUR CODE                             #
		############################################################################
		...
		if self.num_layers == 1:
			...
			grads['dx_a'+str(self.num_layers-1)], grads['dw_a'+str(self.num_layers-1)], grads['db_a'+str(self.num_layers-1)] =  affine_backward(dx, self.params['cache_a'+str(self.num_layers-1)])
			for l in reversed(range(0,self.num_layers-1)):
				dx = relu_backward(grads['dx_a'+str(l+1)], self.params['cache_r'+str(l)])
				grads['dx_a'+str(l)], grads['dw_a'+str(l)], grads['db_a'+str(l)] = affine_backward(dx, self.params['cache_a'+str(l)])
		############################################################################
		#                             END OF YOUR CODE                             #
		############################################################################
