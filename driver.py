...
'''
# If you want to see all result you can comment out this part and comment next part
node_num = 30
input_num = 2
num_split = 5
epoch = 500
...
# Parameters are intilized for 3 hidden layer and 1 learning rate and node number 30 epoch 500
node_num = 30
input_num = 2
num_split = 5
epoch = 500

hidden_layer_number = [[node_num, node_num, node_num]]
data_set_name = ['./set1.dat', './set2.dat']
learning_rates = [5e-4]
	...
	for j, hidden in enumerate(hidden_layer_number):
		...
		for k, rate in enumerate(learning_rates):
			...
			for l in range(num_split):
				...
				train_data = np.concatenate((X_train[0 : X_train.shape[0]/num_split*l], X_train[X_train.shape[0]/num_split*(l+1) : X_train.shape[0]]), axis=0)
				train_label = np.concatenate((Y_train[0 : Y_train.shape[0]/num_split*l], Y_train[Y_train.shape[0]/num_split*(l+1) : Y_train.shape[0]]), axis=0)

				# calculates the train and validate value for all splits
				loss_train, loss_valid = net.train_validate(train_data, train_label, validation_data, validation_label, epoch , rate)
			...
			end_rate = time.time()
			print('Finish train and validate for rate ' + str(rate) + ' time: ' + str(end_rate-begin_rate) + ' seconds\n')

			'''
			# plots the mean value of loss and validation results for all rates
			plt.plot(np.arange(epoch), loss_train_mean[k], label='train')
			plt.plot(np.arange(epoch), loss_valid_mean[k], label='validation')
			plt.title('Loss Function for ' + str(len(hidden)) + ' hidden layer rate ' + str(learning_rates[k]))
		...
		plt.title('Loss Function for ' + str(len(hidden)) + ' hidden layer rate ' + str(learning_rates[np.argmin(final_loss)]))
		plt.legend(loc='upper right')
		plt.show()

		# trains the new ann with all train data
		_rate = learning_rates[np.argmin(final_loss)]
		...
		print('Relative error: ' + str( np.max(np.abs(Y_test - pred_test) / (np.maximum(1e-8, np.abs(Y_test) + np.abs(pred_test))))))
		print('\n')

		end_layers = time.time()
		print('Finish Train and Test for ' + str(len(hidden)) + ' hidden layer time: ' + str(end_layers-begin_layers) + ' seconds\n')

	end_dataset = time.time()
	print('Finish train and test for ' + data_set_name[i][2:6] + ' time: ' + str(end_dataset-begin_dataset) + ' seconds\n')
